...
async fn deploy_everything_on_localnet(
    ...
    if let Some(config_output) = config_output {
        ...
        if let StoredValue::Account(account) = result.stored_value {
            ...
            let token_package_hash =
                if let StoredValue::Contract(contract) = token_package_hash.stored_value {
                    contract.contract_package_hash()
                } else {
                    anyhow::bail!("couldn't get token contract")
                };
